...
def lambda_handler(event, context):
    ...
    # Initialize an empty list where information about the chat rooms that are not accepted for work will be stored.
    non_accepted_chat_rooms_entries = list()
    ...
    for channel_id in channels_ids:
        ...
        # Use the 'extend()' method to add 'entries' at the end of 'non_accepted_chat_rooms'.
        non_accepted_chat_rooms_entries.extend(entries)

    # Initialize an empty list.
    non_accepted_chat_rooms = list()

    # Check for data in the list that we received from the database.
    if len(non_accepted_chat_rooms_entries) != 0 or non_accepted_chat_rooms_entries is not None:
        # Create the list of IDs for all clients.
        clients_ids = list()
        for entry in non_accepted_chat_rooms_entries:
        ...
        # Analyze the list with data about non accepted chat rooms received from the database.
        for entry in non_accepted_chat_rooms_entries:
            non_accepted_chat_room = dict()
            for key, value in entry.items():
                ...
                if key == "client_id":
                    ...
                    non_accepted_chat_room["channel"] = channels_and_channels_types_storage[value]
                else:
                    non_accepted_chat_room[utils.camel_case(key)] = value
            non_accepted_chat_rooms.append(non_accepted_chat_room)

    # Return the list of non accepted chat rooms.
