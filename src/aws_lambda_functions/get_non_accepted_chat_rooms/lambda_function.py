...
def check_input_arguments(**kwargs) -> None:
    ...
    required_arguments = ["organizationId", "channelsIds"]
...
def get_non_accepted_chat_rooms_data(**kwargs) -> List[Dict[AnyStr, Any]]:
    ...
    # Prepare the CQL query that gets a list of non accepted chat rooms for the specific organization.
    ...
    # Define the empty list to store information about non accepted chat rooms from all channels.
    non_accepted_chat_rooms_data = []

    # For each channel, get all non accepted chat rooms from the database.
    for channel_id in channels_ids:
        ...
        try:
            non_accepted_chat_rooms = cassandra_connection.execute(cql_statement, cql_arguments)
        except Exception as error:
            logger.error(error)
            raise Exception(error)

        # Add the results to the general list.
        non_accepted_chat_rooms_data.extend(non_accepted_chat_rooms)

    # Return the list of non accepted chat rooms from all channels.
    return non_accepted_chat_rooms_data
...
def analyze_and_format_non_accepted_chat_rooms_data(**kwargs) -> List[Dict[AnyStr, Any]]:
    # Check if the input dictionary has all the necessary keys.
    try:
        non_accepted_chat_rooms_data = kwargs["non_accepted_chat_rooms_data"]
    except KeyError as error:
        logger.error(error)
        raise Exception(error)
    try:
    ...
    # Format the non accepted chat rooms data.
    non_accepted_chat_rooms = []
    for non_accepted_chat_room_data in non_accepted_chat_rooms_data:
        non_accepted_chat_room = {}
        for key, value in non_accepted_chat_room_data.items():
            ...
            if key.startswith("client_id"):
                non_accepted_chat_room["client"] = clients_storage[value]
            elif key.startswith("channel_id"):
                non_accepted_chat_room["channel"] = channels_storage[value]
            else:
                non_accepted_chat_room[utils.camel_case(key)] = value
        non_accepted_chat_rooms.append(non_accepted_chat_room)

    # Return analyzed and formatted data.
    return non_accepted_chat_rooms
...
def lambda_handler(event, context):
    ...
    organization_id = uuid.UUID(input_arguments["organization_id"])
    ...
    # Get all non accepted chat rooms from different channels.
    non_accepted_chat_rooms_data = get_non_accepted_chat_rooms_data(
        cassandra_connection=cassandra_connection,
        cql_arguments={
    ...
    if non_accepted_chat_rooms_data:
        # Create the list of IDs for all clients.
        clients_ids = [str(item["client_id"]) for item in non_accepted_chat_rooms_data]
    ...
    # Return the list of all non accepted chat rooms from different channels.
    return non_accepted_chat_rooms
