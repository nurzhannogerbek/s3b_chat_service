    ...
    for channel_id in channels_ids:
        ...
        try:
            ...
            if paging_state is not None:
                ...
                entries = cassandra_connection.execute(
                    statement,
                    paging_state=binascii.unhexlify(paging_state)
                )
            else:
                entries = cassandra_connection.execute(statement)
        except Exception as error:
            logger.error(error)
            sys.exit(1)

        # Use the 'extend()' method to add 'entries' at the end of 'completed_chat_rooms_entries'.
        completed_chat_rooms_entries.extend(entries)

    # Initialize an empty list.
    completed_chat_rooms = list()

    # Check for data in the list that we received from the database.
    if len(completed_chat_rooms_entries) != 0 or completed_chat_rooms_entries is not None:
        # Create the list of IDs for all clients.
        clients_ids = list()
        for entry in completed_chat_rooms_entries:
        ...
        # Analyze the list with data about completed chat rooms received from the database.
        for entry in completed_chat_rooms_entries:
            completed_chat_room = dict()
            for key, value in entry.items():
                ...
                if key == "client_id":
                    ...
                    completed_chat_room["channel"] = channels_and_channels_types_storage[value]
                else:
                    completed_chat_room[utils.camel_case(key)] = value
            completed_chat_rooms.append(completed_chat_room)

    # Return the list of completed chat rooms.
