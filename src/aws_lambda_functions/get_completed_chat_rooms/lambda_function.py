...
def get_completed_chat_rooms_data(**kwargs) -> List[Dict[AnyStr, Any]]:
    ...
    # Prepare the CQL query that gets a list of competed chat rooms for the specific channel.
    ...
    # For each channel, get all completed chat rooms from the database.
    for channel_id in channels_ids:
        ...
        completed_chat_rooms_data.extend(completed_chat_rooms)

    # Return the list of completed chat rooms from all channels.
    return completed_chat_rooms_data
...
def analyze_and_format_completed_chat_rooms_data(**kwargs) -> List[Dict[AnyStr, Any]]:
    # Check if the input dictionary has all the necessary keys.
    try:
        completed_chat_rooms_data = kwargs["completed_chat_rooms_data"]
    except KeyError as error:
        logger.error(error)
        raise Exception(error)
    try:
    ...
    # Format the completed chat rooms data.
    completed_chat_rooms = []
    for completed_chat_room_data in completed_chat_rooms_data:
        completed_chat_room = {}
        for key, value in completed_chat_room_data.items():
            ...
            if key.startswith("client_id"):
                completed_chat_room["client"] = clients_storage[value]
            elif key.startswith("channel_id"):
                completed_chat_room["channel"] = channels_storage[value]
            else:
                completed_chat_room[utils.camel_case(key)] = value
        completed_chat_rooms.append(completed_chat_room)

    # Return analyzed and formatted data.
    return completed_chat_rooms
...
def lambda_handler(event, context):
    ...
    # Get all completed chat rooms from different channels.
    completed_chat_rooms_data = get_completed_chat_rooms_data(
        cassandra_connection=cassandra_connection,
        cql_arguments={
    ...
    if completed_chat_rooms_data:
        # Create the list of IDs for all clients.
        clients_ids = [str(item["client_id"]) for item in completed_chat_rooms_data]
        ...
        completed_chat_rooms_data = sorted(
            completed_chat_rooms_data,
            key=lambda x: x["last_message_date_time"],
            reverse=True
        )

        # Define the variable that stores information about all completed chat rooms from different channels.
        completed_chat_rooms = analyze_and_format_completed_chat_rooms_data(
            completed_chat_rooms_data=completed_chat_rooms_data,
    ...
    # Return the list of all completed chat rooms from different channels.
    return completed_chat_rooms
